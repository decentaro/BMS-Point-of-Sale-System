...
function register(ipcMain, apiConfigManager) {
    ...
    ipcMain.handle('print-receipt', async (event, receiptContent, logoPath = null, businessNameOverride = null, copies = 1) => {
        try {
            const printer = findPrinterDevice()
            if (!printer) {
            ...
            const buffer = Buffer.from(thermalReceiptContent, 'binary')
            for (let i = 0; i < copyCount; i++) {
                await sendRawToPrinter(printer, buffer)
            }
            return { success: true, message: `Receipt printed to ${printer.name}${copyCount > 1 ? ` (${copyCount} copies)` : ''}` }
        } catch (error) {
