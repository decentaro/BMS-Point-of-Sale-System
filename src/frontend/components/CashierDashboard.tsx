import React from 'react'
import { useNavigate } from 'react-router-dom'
import SessionGuard from './SessionGuard'
import DashboardShell, { SectionCard } from './DashboardShell'
import { CASHIER_CARDS } from '../config/nav-data'
import NavCardButton from './ui/NavCardButton'

const CashierDashboard: React.FC = () => {
  const navigate = useNavigate()

  return (
    <SessionGuard requiredRole="Cashier">
      <DashboardShell title="Cashier Dashboard">
        <div className="h-full flex flex-col justify-center">
          <SectionCard label="Quick Actions">
            <div className="grid grid-cols-3 gap-4" style={{ height: 200 }}>
              {CASHIER_CARDS.map(card => (
                <NavCardButton key={card.route} card={card} onClick={() => navigate(card.route)} />
              ))}
            </div>
          </SectionCard>
        </div>
      </DashboardShell>
    </SessionGuard>
  )
}

export default CashierDashboard
